...
from lib.Models.network import discriminator
...
class QLearner:
    def __init__(self,
                 ...
                 qstore=None,   
                 replaydict = None,
                 replay_dictionary = pd.DataFrame(columns=['net_disc',
                                                         'input_size',
                                                         'reward',
                                                         'epsilon',
                                                         'train_flag'])):
    ...
    def generate_net(self, epsilon = None, dataset = None): 
        ...
        train_flag = True
        if (net_string in self.replay_dictionary['net_disc'].values) and (int(state_list[-1].fc_size) in self.replay_dictionary[self.replay_dictionary['net_disc']==net_string]['input_size'].values):
            reward = self.replay_dictionary[(self.replay_dictionary['net_disc']==net_string) & (self.replay_dictionary['input_size']==int(state_list[-1].fc_size))]['reward'].values[0]
            train_flag = self.replay_dictionary[(self.replay_dictionary['net_disc']==net_string) & (self.replay_dictionary['input_size']==int(state_list[-1].fc_size))]['train_flag'].values[0]
            self.replay_dictionary = self.replay_dictionary.append(pd.DataFrame([[net_string, reward,\
                                        self.epsilon, train_flag, int(state_list[-1].fc_size)]], columns=['net_disc', 'reward', \
                                        'epsilon', 'train_flag', 'input_size']), ignore_index = True)
            self.count += 1
            ...
            reward, train_flag = self.__train_val_net(state_list, self.state_space_parameters, dataset)
    ...
    def __train_val_net(self, state_list, state_space_parameters, dataset):
        ...
        gen_model = net(state_list, state_space_parameters, net_input, self.args.batch_norm, self.args.drop_out_drop)
        disc_model = discriminator(state_space_parameters, net_input, self.args.discriminator_classes)
        print(gen_model)
        print('-' * 80)
        print('Input size: {}'.format(gen_model.input_size))
        print('-' * 80)
        print ('Estimated total gpu usage of model: {gpu_usage:.4f} GB'.format(gpu_usage = gen_model.gpu_usage))
        ...
        if ((self.gpu_mem_0.total_mem - self.gpu_mem_0.total_mem*self.gpu_mem_0.get_mem_util())/1024.) < ((3./float(self.args.no_gpus)*model_activations_gpu) + 1): 
            ...
            return [None] * 2
        elif not (gen_model.convT_no>0 or gen_model.wrnT_bb_no>0 or gen_model.fc_no>0):
            del gen_model, disc_model
            return [None] * 2
        ...
        gen_optimizer = optim.SGD(filter(lambda p: p.requires_grad, gen_model.parameters()),
                               lr=self.args.learning_rate,
                               momentum=self.args.momentum,  weight_decay=self.args.weight_decay)
        disc_optimizer = optim.SGD(filter(lambda p: p.requires_grad, disc_model.parameters()),
                               lr=self.args.learning_rate,
                               momentum=self.args.momentum,  weight_decay=self.args.weight_decay)
    ...
    def sample_replay_for_update(self):
        net = self.replay_dictionary.iloc[-1]['net_disc']
        input_size = self.replay_dictionary.iloc[-1]['input_size']
        reward = self.replay_dictionary.iloc[-1]['reward']
        state_list = self.stringutils.convert_model_string_to_states(cnn_parse('net', net))
        self.update_q_value_sequence(state_list, self.accuracy_to_reward(reward), input_size)
        ...
        for i in range(self.state_space_parameters.replay_number -1):
            net = np.random.choice(self.replay_dictionary['net_disc'])
            input_size = np.random.choice(self.replay_dictionary[(self.replay_dictionary['net_disc'] == net)]['input_size'])
            reward = self.replay_dictionary[(self.replay_dictionary['net_disc'] == net) & (self.replay_dictionary['input_size'] == input_size)]['reward'].values[0]
            state_list = self.stringutils.convert_model_string_to_states(cnn_parse('net', net))
            self.update_q_value_sequence(state_list, self.accuracy_to_reward(reward), input_size)
    ...
    def update_q_value_sequence(self, states, termination_reward, input_size):
        states[-1].fc_size = input_size
    ...
    def _update_q_value(self, start_state, to_state, reward):
        ...
        #                                         self.args.learning_rate * (reward + self.state_space_parameters.discount_factor * max_over_next_states - values[actions.index(action_between_states)])
